...
public class Uploader {
	 ...
	 public static HttpHeaders getHttpHeaders(String fileName) throws Exception {
	  ...
	  MediaType mediaType = MimeType.getMediaType(fileName); // 파일타입 확인
	 ...
	 public static Map<String, String> uploadFile(MultipartFile file, HttpServletRequest request) throws Exception {
	 ...
	 public static String getRootPath(String fileName, HttpServletRequest request) {
	  ...
	  MediaType mediaType = MimeType.getMediaType(fileName); // 파일타입 확인
	  if (mediaType != null)
	   return request.getSession().getServletContext().getRealPath(rootPath + "/img"); // 이미지 파일 경로
